    ...
    for i in range(len(data)):
        ...
        for j in range(len(have)):
            ...
            for k in range(len(all_var)):
                mid_p = mid_p * f(p[j][k]['u'], p[j][k]['s']+0.01, data[i][k])*(abs(cc[k][5])+message[k])/2
            most_P[j]=mid_p
        mm=m_p(most_P)
        pr[i][mm]=1
    return pr
...
def pred2(data,p,all_var,p_first):
    ...
    for i in range(len(data)):
        ...
        for j in range(len(have)):
            mid_p=p_first[j]
            for k in range(len(all_var)):
                mid_p = mid_p * f(p[j][k]['u'], p[j][k]['s']+0.01, data[i][k])#*(abs(cc[k][9])+message[k])/2
            most_P[j]=mid_p
        mm=m_p(most_P)
        pr[i][mm]=1
    return pr
...
t,v=lable_split(data)
pca_sk = PCA(n_components=5)
t = pca_sk.fit_transform(t)
