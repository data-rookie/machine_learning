...
while (maxIter > 0):
    ...
    for i in range(m):
        ...
        g = predictY * (1 - predictY) * (train_lab[i] - predictY)
        e = b * (1 - b) * ((np.dot(w, g.T)).T)
        mid_w=np.dot(b.reshape((q, 1)), g.reshape((1, l)))
        all_grade_w=all_grade_w+np.dot(mid_w,mid_w.T)#############################################
        w =w + (eta/math.sqrt(sum_of_juzhen(all_grade_w)+ee)) * np.dot(b.reshape((q, 1)), g.reshape((1, l)))
        all_grade_theta=all_grade_theta+np.dot(g,g.T)
        theta = theta - (eta/math.sqrt(all_grade_theta+ee)) * g
        mid_v=np.dot(train_var[i].reshape((d, 1)), e.reshape((1, q)))
        all_grade_v=all_grade_v+np.dot(mid_v,mid_v.T)
        v =v + (eta/math.sqrt(sum_of_juzhen(all_grade_v)+ee)) * np.dot(train_var[i].reshape((d, 1)), e.reshape((1, q)))
        all_grade_gamma=all_grade_gamma+np.dot(e,e.T)
        gamma =gamma - (eta/math.sqrt(all_grade_gamma+ee)) * e
    if len(EE3)<long:
        EE3.append(sumE)
    # print(sumE)
...
while (maxIter > 0):
    ...
    for i in range(m):
        ...
        e = b * (1 - b) * ((np.dot(w, g.T)).T)
        mid_w=np.dot(b.reshape((q, 1)), g.reshape((1, l)))
        all_grade_w=yy*all_grade_w+(1-yy)*np.dot(mid_w,mid_w.T)#############################################
        w =w + (eta/math.sqrt(sum_of_juzhen(all_grade_w)+ee)) * np.dot(b.reshape((q, 1)), g.reshape((1, l)))
        all_grade_theta=yy*all_grade_theta+(1-yy)*np.dot(g,g.T)
        theta = theta - (eta/math.sqrt(all_grade_theta+ee)) * g
        mid_v=np.dot(train_var[i].reshape((d, 1)), e.reshape((1, q)))
        all_grade_v=yy*all_grade_v+(1-yy)*np.dot(mid_v,mid_v.T)
        v =v + (eta/math.sqrt(sum_of_juzhen(all_grade_v)+ee)) * np.dot(train_var[i].reshape((d, 1)), e.reshape((1, q)))
        all_grade_gamma=yy*all_grade_gamma+(1-yy)*np.dot(e,e.T)
        gamma =gamma - (eta/math.sqrt(all_grade_gamma+ee)) * e
    if len(EE4)<long:
        EE4.append(sumE)
...
while (maxIter > 0):
    ...
    for i in range(m):
        ...
        all_grade_w=yy*all_grade_w+(1-yy)*np.dot(mid_w,mid_w.T)
        try:
            eta=sum_of_juzhen(changed_w)
        except:
            eta=changed_w
        w =w + ((eta+ee)/math.sqrt(sum_of_juzhen(all_grade_w)+ee)) * np.dot(b.reshape((q, 1)), g.reshape((1, l)))
        m_w=((eta+ee)/math.sqrt(sum_of_juzhen(all_grade_w)+ee)) * np.dot(b.reshape((q, 1)), g.reshape((1, l)))
        changed_w=yy*changed_w+(1-yy)*np.dot(m_w,m_w.T)
        #theta
        all_grade_theta=yy*all_grade_theta+(1-yy)*np.dot(g,g.T)
        try:
            eta=sum_of_juzhen(changed_theta)
        except:
            eta=changed_theta
        theta = theta - ((eta+ee)/math.sqrt(all_grade_theta+ee)) * g
        m_theta=(((eta+ee)/math.sqrt(all_grade_theta+ee)) * g)
        changed_theta=yy*changed_theta+(1-yy)*np.dot(m_theta,m_theta.T)
        #v
        mid_v=np.dot(train_var[i].reshape((d, 1)), e.reshape((1, q)))
        all_grade_v=yy*all_grade_v+(1-yy)*np.dot(mid_v,mid_v.T)
        try:
            eta=sum_of_juzhen(changed_v)
        except:
            eta=changed_v
        v =v + ((eta+ee)/math.sqrt(sum_of_juzhen(all_grade_v)+ee)) * np.dot(train_var[i].reshape((d, 1)), e.reshape((1, q)))
        m_v=((eta+ee)/math.sqrt(sum_of_juzhen(all_grade_v)+ee)) * np.dot(train_var[i].reshape((d, 1)), e.reshape((1, q)))
        changed_v=yy*changed_v+(1-yy)*np.dot(m_v,m_v.T)
        #gamma
        all_grade_gamma=yy*all_grade_gamma+(1-yy)*np.dot(e,e.T)
        try:
            eta=sum_of_juzhen(changed_gamma)
        except:
            eta=changed_gamma
        gamma =gamma - ((eta+ee)/math.sqrt(all_grade_gamma+ee)) * e
        m_gamma=((eta+ee)/math.sqrt(all_grade_gamma+ee)) * e
        changed_gamma=yy*changed_gamma+(1-yy)*np.dot(m_gamma,m_gamma.T)
    if len(EE5)<long:
        EE5.append(sumE)
...
#
plt.plot(XX, EE1,label=u"标注bp")
plt.plot(XX, EE2,label=u"累计bp")
plt.plot(XX, EE3,label=u"AdaGrad")
plt.plot(XX, EE4,label=u"Adadelta1")
plt.plot(XX, EE5,label=u"Adadelta2")
plt.plot(XX, EE6,label=u"decay衰减少")
plt.legend()#添加图例
plt.show()
