...
def chooseBestFeatureToSplit(dataSet, labels):
    ...
    for i in range(numFeatures):
        ...
        print('当前特征值为：' + str(labels[i]) + '，对应的信息增益值为：' + str(infoGain))
        # 如果当前的信息增益比原来的大
    ...
    print('信息增益最大的特征为：' + str(labels[bestFeature]))
...
def fenlei(inputTree, labels, testVec):

    firstStr=list(inputTree.keys())[0]
    secondDict=inputTree[firstStr]
    featIndex=labels.index(firstStr.split('<')[0])
    if testVec[featIndex]<float(firstStr.split('<')[1][:-1:]):
        key=list(secondDict.keys())[0]
    else:
        key = list(secondDict.keys())[1]
    if type(secondDict[key]).__name__ == 'dict':
        classlabel = fenlei(secondDict[key], labels, testVec)
    else:
...
def TestAccuracy(inputTree,labels,DataSet):
    ...
    # 遍历数据集中所有对测试用例
    for feat in DataSet:
        predict_label = fenlei(inputTree, labels, feat)
...
def data_split(data,train_size=0.7,random=False):
    if random:
...
if __name__ == '__main__':
    ...
    处理连续值时候的决策树
    """
    dataSet = pd.read_csv(r'C:\Users\cjn\Desktop\pycccccc\机器学习课设\data\winequality-red.csv',header=0,index_col=None,encoding='gbk',sep=';')
    trainSet,testdataSet = data_split(dataSet,train_size=0.7,random=True)
    labels = ['fixed acidity','volatile acidity','citric acid','residual sugar','chlorides','free sulfur dioxide','total sulfur dioxide','density'
    ...
    myTree = createTree(trainSet, labels)
